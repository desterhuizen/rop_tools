...
Categories:
  ...
  call, jmp, ret, conditional, syscall, interrupt, string_ops, other
    ...
    parser.add_argument('-f', '--file', required=True,
                       help='Path to rp++ output file')
    parser.add_argument('-i', '--instruction',
                       help='Filter by instruction name')
    parser.add_argument('-p', '--position', choices=['any', 'first', 'last'],
                       default='any', help='Position of instruction to match')
    parser.add_argument('-b', '--bad-chars',
                       help='Bad characters to filter (e.g., "\\x00\\x0a" or "00,0a,0d")')
    parser.add_argument('-m', '--max-instructions', type=int,
                       help='Maximum number of instructions in gadget')
    parser.add_argument('-g', '--group', choices=['first', 'last', 'category', 'register', 'modified-register', 'dereferenced-register', 'category-register'],
    ...
    parser.add_argument('--deref', '--dereferenced',
    ...
    parser.add_argument('-s', '--stats', action='store_true',
                       help='Show statistics about gadgets')
    parser.add_argument('--show-category', action='store_true',
                       help='Display category for each gadget')
    ...
    parser.add_argument('--sort', choices=['count', 'address'],
                       default='count', help='Sort gadgets by count (default) or address')
    ...
    parser.add_argument('--keep-bad-instructions', action='store_true',
    ...
    if args.category:
        ...
        filtered_gadgets = [g for g in filtered_gadgets
                           if rop_parser.categorize_gadget(g) == args.category]
        print(f"[*] Found {len(filtered_gadgets)} gadgets in category '{args.category}' "
    ...
    if args.instruction:
        ...
        print(f"[*] Found {len(filtered_gadgets)} gadgets with '{args.instruction}' "
              f"({args.position} position)\n")

    if args.regex:
        filtered_gadgets = [g for g in filtered_gadgets
                           if re.search(args.regex, g.get_instruction_chain(), re.IGNORECASE)]
        print(f"[*] Found {len(filtered_gadgets)} gadgets matching regex '{args.regex}'\n")

    if args.exclude:
        before = len(filtered_gadgets)
        filtered_gadgets = [g for g in filtered_gadgets
                           if not re.search(args.exclude, g.get_instruction_chain(), re.IGNORECASE)]
        print(f"[*] Excluded {before - len(filtered_gadgets)} gadgets matching pattern '{args.exclude}' "
              f"({len(filtered_gadgets)} remaining)\n")

    if args.max_instructions:
        before = len(filtered_gadgets)
        filtered_gadgets = [g for g in filtered_gadgets
                           if len(g.instructions) <= args.max_instructions]
        print(f"[*] Filtered to {len(filtered_gadgets)} gadgets with "
    ...
    if args.bad_chars:
        ...
        bad_chars_str = args.bad_chars.replace('\\x', ' ').replace(',', ' ').strip().split()
        bad_char_set = {c.lower() for c in bad_chars_str}
        filtered_gadgets = [g for g in filtered_gadgets
                           if not g.contains_bad_chars(bad_char_set)]
        print(f"[*] Filtered to {len(filtered_gadgets)} gadgets without bad chars "
              f"(removed {before - len(filtered_gadgets)})\n")

    if args.register:
        before = len(filtered_gadgets)
        # Use the parser's filter method which respects modified_only flag
        rop_parser.gadgets = filtered_gadgets  # Temporarily set gadgets to filtered list
        filtered_gadgets = rop_parser.filter_by_register(args.register, args.modified_only)
        rop_parser.gadgets = rop_parser.gadgets  # Restore original
        mod_str = "modify" if args.modified_only else "affect"
        print(f"[*] Found {len(filtered_gadgets)} gadgets that {mod_str} register '{args.register}' "
              f"(removed {before - len(filtered_gadgets)})\n")
    ...
    if args.deref is not None:
        ...
        if deref_reg:
            print(f"[*] Found {len(filtered_gadgets)} gadgets with dereferenced register '[{deref_reg}]' "
                  f"(removed {before - len(filtered_gadgets)})\n")
        else:
            print(f"[*] Found {len(filtered_gadgets)} gadgets with dereferenced registers "
                  f"(removed {before - len(filtered_gadgets)})\n")
    ...
    if not args.keep_bad_instructions:
        ...
        filtered_gadgets = [g for g in filtered_gadgets if not contains_bad_instruction(g)]
        print(f"[*] Filtered out {before - len(filtered_gadgets)} gadgets with bad instructions "
              f"({len(filtered_gadgets)} remaining)\n")
    ...
    if args.offset:
        try:
            base_address = int(args.offset, 16) if args.offset.startswith('0x') else int(args.offset, 10)
        except ValueError:
            print(f"[!] Error: Invalid base address format '{args.offset}'. Use hex (e.g., 0x10000000) or decimal format.", file=sys.stderr)
            sys.exit(1)
    ...
    if args.stats:
        ...
        if args.group == 'category':
            ...
            for category, gadgets in sorted(groups.items(), key=lambda x: len(x[1]), reverse=True):
                category_style = get_category_style(category)
                printer.print_section(f"\n--- {category} ({len(gadgets)} gadgets) ---", category_style)
                print_gadgets(gadgets, args.limit, rop_parser, args.show_category, args.show_count, highlight_pattern, base_address)
        elif args.group == 'register':
            groups = rop_parser.group_by_affected_register(filtered_gadgets)

            printer.print_section("=== Grouped by affected register ===\n", "bold green")

            for reg, gadgets in sorted(groups.items(), key=lambda x: len(x[1]), reverse=True):
            ...
            printer.print_section("=== Grouped by modified register ===\n", "bold green")
            ...
            printer.print_section("=== Grouped by dereferenced register ===\n", "bold green")
            ...
            printer.print_section("=== Grouped by category, then by modified register ===\n", "bold green")

            # Sort categories by total gadget count
            category_totals = {cat: sum(len(gads) for gads in regs.values())
            ...
            for category in sorted(category_totals.keys(), key=lambda x: category_totals[x], reverse=True):
                ...
                category_style = get_category_style(category)
                printer.print_section(f"\n{'='*70}", category_style)
                printer.print_section(f"  {category.upper()} ({total_in_cat} total gadgets)", category_style)
                printer.print_section(f"{'='*70}\n", category_style)

                for reg, gadgets in sorted(reg_groups.items(), key=lambda x: len(x[1]), reverse=True):
            ...
            for inst, gadgets in sorted(groups.items(), key=lambda x: len(x[1]), reverse=True):
        ...
        printer.print_section(f"=== Results ({len(filtered_gadgets)} gadgets) ===\n", "bold green")
        print_gadgets(filtered_gadgets, args.limit, rop_parser, args.show_category, args.show_count, highlight_pattern, base_address)
