...
def build_worksheet_view(ws: Dict[str, Any]) -> Group:
    ...
    auto_status = "[green]ON[/green]" if ws.get("auto_gadget", True) else "[red]OFF[/red]"
    log_manual_status = "[green]ON[/green]" if ws.get("log_manual", True) else "[red]OFF[/red]"
    ...
    reg_table = Table(title="REGISTERS", box=box.SIMPLE, show_header=True, header_style="bold magenta")
    ...
    eip_display = f"[bold green]{eip_val}[/bold green]" if eip_val and eip_val != "0x00000000" else "[dim]{eip_val}[/dim]"
    ...
    stack_table = Table(title="STACK", box=box.SIMPLE, show_header=True, header_style="bold magenta")
    ...
    if ws["stack"]:
        ...
        sorted_stack = sorted(ws["stack"].items(), key=lambda x: int(x[0], 16) if x[0].startswith("+") or x[0].startswith("-") else int(x[0], 16))
    ...
    # Named Values
    if ws["named"]:
        named_table = Table(title="NAMED VALUES", box=box.SIMPLE, show_header=True, header_style="bold magenta")
    ...
    # Gadget Library
    if ws["gadgets"]:
        gadgets_table = Table(title="GADGET LIBRARY", box=box.SIMPLE, show_header=True, header_style="bold magenta")
        ...
        sorted_gadgets = sorted(ws["gadgets"].items(), key=lambda x: int(x[0], 16) if x[0].startswith("0x") else 0)
    ...
    # ROP Chain
    if ws["chain"]:
        chain_table = Table(title="ROP CHAIN", box=box.SIMPLE, show_header=True, header_style="bold magenta")
    ...
    if ws["execution_log"]:
        log_table = Table(title="EXECUTION LOG (Last 10)", box=box.SIMPLE, show_header=True, header_style="bold magenta")
