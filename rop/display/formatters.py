...
def print_gadget_colored(gadget: Gadget, parser: ROPGadgetParser, show_category: bool = False, show_count: bool = False, highlight_pattern: Optional[str] = None, base_address: Optional[int] = None):
    ...
    if not printer.enabled:
        ...
        # Calculate offset for non-colored output
        offset_str = ""
    ...
    if len(parts) == 2:
        ...
        # Apply highlighting to the instruction part if pattern provided
        ...
        # Add offset if base_address provided
        if base_address is not None:
            gadget_addr = int(gadget.address, 16)
            offset = gadget_addr - base_address
            output.append(f" (+0x{offset:x})", style="#ff8800")

        # Add the rest (instructions)
        output.append(":")
        if isinstance(rest, Text):
            output.append(rest)
        else:
            output.append(rest)
