...
; ==========================================================================
    ...
    ; [ebp+0x08] = (first resolved API - TerminateProcess)
    ; [ebp+0x0c] = (second resolved API - CreateProcessA)
    ; [ebp+0x10] = LoadLibraryA
    ; kernel32 APIs are pre-resolved in boilerplate
    ...
    call dword ptr [ebp+0x0c]     ; Call CreateProcessA from pre-resolve

    ; ===== Exit with TerminateProcess (pre-resolved) =====
    xor ecx, ecx
    push ecx                      ; uExitCode = 0
    push 0xFFFFFFFF               ; hProcess = -1 (current process)
    call dword ptr [ebp+0x08]     ; Call TerminateProcess from pre-resolve
